<template>
  <div id="vote">
    <div class="alertContainer">
      ...
      >{{info}} 这条消息将在 {{dismissCountDown}} 秒后消失...</b-alert>
    </div>
    <ve-pie :data="chartData" :settings="chartSettings"></ve-pie>
    <ul>
      <li v-for="(user,key) of classmates" :key="user.id">
        <font>{{user.name}}-支持率：{{user.approval_rate}}</font>
        <button class="form-control" @click="add(key,user)">支持</button>
      </li>
...
export default {
  ...
  methods: {
    ...
    add(index, item) {
      this.id = index;
      this.axios
        .patch("/add", { id: this.id, account: this.$cookies.get("isLogin") })
        .then(
          res => {
            if (
              res.data == "与服务器断开连接" ||
              res.data == "您已投票了，请勿重复投票" ||
              res.data == "提交成功"
            ) {
